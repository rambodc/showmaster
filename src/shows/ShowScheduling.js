...
function ScheduleHeader({ meta, selectedDay, dayItems, dayWarnings, canEdit, onAddItem, onEditSetup, onEditDay }) {
  return (
    <section className="show-hero-card schedule-hero-card">
      <div className="schedule-hero-top">
        ...
        <div className="schedule-hero-actions">
          {canEdit ? (
            <button className="show-btn" type="button" onClick={onAddItem}>
      ...
      <div className="schedule-hero-metrics">
        <article className="schedule-metric-card">
          <span className="schedule-metric-label">Status</span>
          <strong>{getStatusMeta(meta.status).label}</strong>
          <p>{meta.timezone}</p>
        </article>
        <article className="schedule-metric-card">
          <span className="schedule-metric-label">Selected day</span>
        ...
        <article className="schedule-metric-card">
          <span className="schedule-metric-label">Daily load</span>
...
function FestivalDayStrip({ days, selectedDayId, itemsByDay, meta, getConflicts, onSelect }) {
  return (
    <section className="show-card schedule-day-strip-card">
        ...
        {days.map((day, index) => {
          ...
          return (
            <button
              key={day.id}
              type="button"
              className={day.id === selectedDayId ? 'schedule-day-pill active' : 'schedule-day-pill'}
              onClick={() => onSelect(day.id)}
            >
              <span className="schedule-day-pill-top">
...
function ScheduleItemDetail({ item, selectedDay, conflicts, canEdit, onEdit, onDelete }) {
  if (!item) {
    return (
  ...
  return (
    <article className="show-card schedule-detail-card">
      {item.imageUrl ? (
