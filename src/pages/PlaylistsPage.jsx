...
import { ArrowDown, ArrowLeft, ArrowUp, Edit3, ListMusic, LoaderCircle, Pause, Play, Plus, Trash2 } from "lucide-react";
...
export default function PlaylistsPage() {
  ...
  return <main className="playlist-page">
    ...
    <header><div className="playlist-heading"><PlaylistArtwork playlist={playlist} compact /><div><span>Private playlist</span><h1>{playlist.name}</h1><p>{tracks.length} saved tracks</p></div></div><div><button disabled={!tracks.length} onClick={() => audio.playTracks(tracks)}><Play fill="currentColor" /> Play all</button><LoadingButton loading={pending === "rename"} loadingLabel="Renaming…" onClick={() => setDialog("rename")}><Edit3 /> Rename</LoadingButton><LoadingButton loading={pending === "delete"} loadingLabel="Deleting…" onClick={() => setDialog("delete")}><Trash2 /> Delete</LoadingButton></div></header>
    {error && <div className="studio-notice error" role="alert">{error}</div>}
    {!tracks.length ? <div className="studio-empty"><ListMusic /><h3>This playlist is empty</h3><p>Add published tracks from a release page or the music player.</p></div> : <section className="playlist-tracks">{tracks.map((track, index) => { const active = audio.isTrackActive(track.id); const trackLoading = active && audio.loading; const playbackState = trackLoading ? " is-loading" : active && audio.playing ? " is-playing" : active ? " is-paused" : ""; return <article className={`${active ? "active" : ""}${playbackState}`} key={track.entryId}><button className="playlist-track__main" onClick={() => active ? audio.toggle() : audio.playTracks(tracks, track.id)} aria-label={`${trackLoading ? "Loading" : active && audio.playing ? "Pause" : "Play"} ${track.title}`}><span className="playlist-track__play">{trackLoading ? <LoaderCircle className="audio-loading-spinner" /> : active && audio.playing ? <Pause fill="currentColor" /> : <Play fill="currentColor" />}</span><span><strong>{track.title}</strong><small>{track.artistName} · {track.releaseTitle}</small></span></button><time>{formatTime(track.durationSeconds || 0)}</time><div className="playlist-track__actions"><AddToPlaylistButton track={track} /><LoadingButton loading={pending === `reorder-${track.entryId}`} disabled={Boolean(pending) || index === 0} onClick={() => reorder(index, -1)} aria-label={`Move ${track.title} up`}><ArrowUp /></LoadingButton><LoadingButton loading={pending === `reorder-${track.entryId}`} disabled={Boolean(pending) || index === tracks.length - 1} onClick={() => reorder(index, 1)} aria-label={`Move ${track.title} down`}><ArrowDown /></LoadingButton><LoadingButton loading={pending === `remove-${track.entryId}`} disabled={Boolean(pending)} onClick={() => run(`remove-${track.entryId}`, async () => { await removeTrackFromPlaylist({ playlistId, entryId: track.entryId }); await playlistContext.load(); await load(); }, "Track removed from playlist.")} aria-label={`Remove ${track.title}`}><Trash2 /></LoadingButton></div></article>; })}</section>}
