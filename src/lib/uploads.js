...
export async function uploadImage({ targetType, targetId, file, onProgress }) { const prepared = await prepareImageUpload({ targetType, targetId, size: file.size, contentType: file.type }); const upload = runUpload(prepared.storagePath, file, onProgress); await upload.promise; return prepared; }
